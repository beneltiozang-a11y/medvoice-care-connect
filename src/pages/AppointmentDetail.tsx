            ...
            {relatedCalls.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-1">
                {relatedCalls[0].symptoms.map((s) => (
                  <span key={s} className="inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-medium bg-secondary text-primary">
                    {s}
                  </span>
                ))}
              </div>
              ...
              <div className="flex gap-1 flex-wrap mt-0.5">
                {patient.allergies.length > 0
                  ? patient.allergies.map((a) => (
                      <span key={a} className="inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium bg-[#FEE2E2] text-destructive">{a}</span>
                    ))
                ...
                {transcript.map((msg, i) => (
                  ...
                  >
                    <span className={`font-semibold text-xs mr-2 ${
                      msg.role === "doctor" ? "text-primary" : "text-foreground"
                    }`}>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {detectedSymptoms.map((s) => (
                          <span key={s} className="inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-medium bg-secondary text-primary">
                            {s}
                          </span>
                        ))}
                      </div>
                        ...
                        <>
                          <div className="flex items-center justify-between">
                            <div className="flex-1">
                              ...
                              <Input
                                ...
                                className="h-8 text-sm bg-card border-input font-medium focus-visible:ring-primary focus-visible:border-primary"
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            <div>
                              ...
                              <Input value={med.dosage} onChange={(e) => updateMed(i, "dosage", e.target.value)} className="h-8 text-xs bg-card border-input focus-visible:ring-primary" placeholder="Dosage" />
                            </div>
                            <div>
                              <label className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1 block">Fréquence</label>
                              <Input value={med.frequency} onChange={(e) => updateMed(i, "frequency", e.target.value)} className="h-8 text-xs bg-card border-input focus-visible:ring-primary" placeholder="Fréquence" />
                            </div>
                            <div>
                              <label className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1 block">Durée</label>
                              <Input value={med.duration} onChange={(e) => updateMed(i, "duration", e.target.value)} className="h-8 text-xs bg-card border-input focus-visible:ring-primary" placeholder="Durée" />
                            </div>
                          </div>
                        </>
                      ) : (
